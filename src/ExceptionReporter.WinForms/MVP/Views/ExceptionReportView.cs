...
namespace ExceptionReporting.MVP.Views
{
  ...
  public partial class ExceptionReportView : Form, IExceptionReportView
  {
	...
	private void PopulateReportInfo(ExceptionReportInfo reportInfo)
	{
	  ...
	  btnSimpleEmail.Text =
		  $"{(reportInfo.SendMethod == ReportSendMethod.WebService ? Resources.Send : "Email")} {(reportInfo.SendMethod == ReportSendMethod.WebService && !string.IsNullOrEmpty(reportInfo.CompanyName) ? string.Format(Resources.to__0_, reportInfo.CompanyName) : reportInfo.CompanyName)}";
	...
	private void AddAssembly(Report.AssemblyRef assembly)
	{
